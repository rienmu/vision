...
from create_dataset import strong_aug, AlbumentationsDataset
from vision.models.my_models import ResNet
...
def evaluate(model, criterion, data_loader, device, print_freq=5, log_suffix=""):
    model.eval()
    metric_logger = utils.MetricLogger(delimiter="  ")
...
def _get_cache_path(filepath):
    ...
    cache_path = os.path.join("~", ".torch", "", "datasets", "imagefolder", h[:10] + ".pt")
...
def load_data(traindir, valdir, args):
    ...
    if args.cache_dataset and os.path.exists(cache_path):
        ...
        #     ),
        # )
        dataset = AlbumentationsDataset(root=traindir, transform=strong_aug(0.7))
    ...
    if args.cache_dataset and os.path.exists(cache_path):
        ...
        #     preprocessing,
        # )
        dataset_test = AlbumentationsDataset(root=valdir, transform=strong_aug(0.7))
        if args.cache_dataset:
...
def main(args):
    ...
    """
    if args.freeze_weight and args.resume is None:
...
def get_args_parser(add_help=True):
    ...
    parser.add_argument("--data-path", default=r"F:\__Earsenal\images/", type=str, help="dataset path")
    ...
    parser.add_argument("--output-dir", default=r"F:\_checkpoint\resnext\\", type=str, help="path to save outputs")
    parser.add_argument("--resume", default=None, type=str, help="path of checkpoint")
    # --resume C:\Users\rien\.cache\torch\hub\checkpoints\resnext50_32x4d-7cdf4587.pth
    ...
    parser.add_argument("--freeze_weight",action="store_true", help="freeze-weight")
